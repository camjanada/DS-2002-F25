...
def _load_lookup_data(lookup_dir):
    ...
    for filename in os.listdir(lookup_dir):
        if filename.endswith('.json'):
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                df['card_market_value'] = df['tcgplayer.prices.holofoil.market'].fillna(
                    df['tcgplayer.prices.normal.market']
                ).fillna(0.0)
                ...
                required_cols = ['card_id', 'card_name', 'card_number',
    ...
    return lookup_df.sort_values(by='card_market_value', ascending=False) \
...
def update_portfolio(inventory_dir, lookup_dir, output_file):
    ...
    merged_df['set_name'] = merged_df['set_name'].fillna('NOT FOUND')
    ...
    print(f"Portfolio update complete. Data save to {output_file}.")
